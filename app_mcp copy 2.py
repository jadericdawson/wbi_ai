...
with st.sidebar:
    ...
    with st.sidebar.expander("🎙️ Microphone (speak a prompt)", expanded=False):
        st.caption("Capture audio, transcribe with Azure Speech, then send as your chat prompt.")

        audio_bytes = None
        detected_ext = "wav"

        # --- Option A: in-page mic component (if installed) ---
        use_mic_component = st.checkbox("Use in-page mic (streamlit_mic_recorder)", value=True)
        if use_mic_component:
            try:
                ...
                rec = mic_recorder(
                    ...
                    key="micblock_sidebar",
                )
                if rec and rec.get("bytes"):
                    audio_bytes = rec["bytes"]   # WAV (PCM) from the component
                    detected_ext = "wav"
                    st.audio(audio_bytes, format="audio/wav")
            except Exception:
                st.info("Mic component not available. Use file upload below.")
                use_mic_component = False

        # --- Option B: upload/record file fallback ---
        if not use_mic_component:
            mic_file = st.file_uploader("Record or upload audio", type=["wav", "m4a", "mp3"], accept_multiple_files=False)
            if mic_file:
                audio_bytes = mic_file.read()
                detected_ext = mic_file.name.split(".")[-1].lower()
                st.audio(audio_bytes)

        # --- Transcribe + send into chat ---
        if st.button("📝 Transcribe & send", use_container_width=True, disabled=(audio_bytes is None)):
            with st.spinner("Transcribing speech → text..."):
                wav16k = ensure_16k_mono_wav(audio_bytes, ext_hint=detected_ext)
                if not wav16k:
                    st.error("Could not prepare audio for transcription.")
                else:
                    text = azure_fast_transcribe_wav_bytes(wav16k, filename=f"mic.{detected_ext}")
                    if not text.strip():
                        st.warning("No speech recognized.")
                    else:
                        active_chat = st.session_state.user_data.get("active_conversation_id")
                        if active_chat:
                            msgs = st.session_state.user_data["conversations"][active_chat]
                            msgs.append({"role": "user", "content": text})
                            save_user_data(st.session_state.user_id, st.session_state.user_data)
                            st.success("Transcript added to chat.")
                            st.rerun()
                        else:
                            st.warning("No active chat found. Start a chat and try again.")
...
with mini_mic_holder:
    # small, right-aligned recorder control
    col_sp, col_mic = st.columns([0.82, 0.18])
    with col_mic:
        # Use the mic component in its smallest form.
        # Many browsers output WebM/Opus; we pass a sensible hint below and also sniff bytes.
        try:
            rec = mic_recorder(
                start_prompt="🎙️",     # single icon
                stop_prompt="⏹️",
                just_once=True,         # click to start, click to stop
                use_container_width=True,
                key="mic_compact_inline",
                # Some versions support `format="webm"`. If unsupported, it's ignored and sniffing still handles it.
                format="webm",          # safe default for browsers
            )
        except TypeError:
            # Older mic_recorder without `format`. Render again without it.
            rec = mic_recorder(
                start_prompt="🎙️",
                stop_prompt="⏹️",
                just_once=True,
        ...
        # When the user stops, we immediately transcribe & insert.
        if rec and rec.get("bytes"):
            raw_bytes = rec["bytes"]
            # Give a helpful but non-blocking audio preview (optional)
            st.audio(raw_bytes)
                ...
                if not wav16k:
                    ...
                    if text.strip():
                        ...
                        if active_chat:
                            ...
                            msgs.append({"role": "user", "content": text})
                            save_user_data(st.session_state.user_id, st.session_state.user_data)
                            st.toast("🎤 Added transcript to chat.")
                            st.rerun()
                        else:
                            st.warning("No active chat found. Start a chat and try again.")
                    else:
        ...
        if persona_type == "agentic":
            ...
            try:
                ...
                with st.spinner("Step 0: Classifying user intent..."):
                    ...
                    response = st.session_state.gpt41_client.chat.completions.create(model=st.session_state.GPT41_DEPLOYMENT, messages=[{"role": "system", "content": router_prompt}], response_format={"type": "json_object"})
                ...
                if intent == "knowledge_base_query":
                    ...
                    with st.spinner("Step 1: Generating comprehensive search query..."):
                        ...
                        response = st.session_state.gpt41_client.chat.completions.create(model=st.session_state.GPT41_DEPLOYMENT, messages=[{"role": "system", "content": initial_system_prompt}, {"role": "user", "content": user_prompt}], response_format={"type": "json_object"})
                    ...
                    with st.spinner("Step 3: Distilling all retrieved data..."):
                        ...
                        response = st.session_state.gpt41_client.chat.completions.create(model=st.session_state.GPT41_DEPLOYMENT, messages=[{"role": "system", "content": distillation_prompt}])
                    ...
                    with st.spinner("Interpreting new fact for confirmation..."):
                        ...
                        response = st.session_state.gpt41_client.chat.completions.create(model=st.session_state.GPT41_DEPLOYMENT, messages=[{"role": "system", "content": fact_extraction_prompt}], response_format={"type": "json_object"})
