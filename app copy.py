...
def inject_brand_and_theme(bg_b64: str | None, word_b64_for_sidebar: str | None, dark: bool = True):
    """Inject CSS for theme, background watermark, and (optionally) sidebar wordmark."""
    light = {
        "--bg": "#f7f7f9",
        "--bg-surface": "#ffffff",
        "--text": "#1f1f1f",
    ...
    dark_theme = {
        ...
        "--border": "#333",
        "--chat-user": "#173a1d",
        "--chat-assistant": "#102a43",
    }
    palette = dark_theme if dark else light
    ...
    .stApp::before {{
        ...
        z-index:-1;
        background:url("data:image/png;base64,{bg_b64}") center/60vmin no-repeat;
        opacity:.12;
        pointer-events:none;
    ...
    side_logo_css = """
    section[data-testid="stSidebar"] .wbi-wordmark-side{
        text-align:center;
        margin:-0.45rem 0 .55rem 0 !important;  /* pull logo up */
        line-height:0;
        pointer-events:none;
    }
    section[data-testid="stSidebar"] .wbi-wordmark-side img{
        max-width:180px;
        height:auto;
        opacity:.95;
        display:block;
    ...
    st.markdown(
        ...
        :root {{
            {css_vars}
            /* active chat button colors */
            --active-chat-bg: rgba(79,158,250,.18);
            --active-chat-bg-hover: rgba(79,158,250,.28);
            --active-chat-border: rgba(79,158,250,.55);
        }}
        ...
        .mini-header {{
            font-size:0.9rem;
            font-weight:600;
            margin:0 0 .2rem 0 !important;  /* tighten spacing */
        ...
        .user-card .u-name  {{ font-weight:600; margin-bottom:0.1rem; }}
        .user-card .u-email {{ opacity:.9; font-size:.78rem; }}
        .user-card .u-model {{ color:var(--text-dim); font-size:.72rem; margin-top:.25rem; }}

        /* chat history scroll */
        .scroll-box{{ max-height:40vh; overflow-y:auto; padding-right:4px; }}

        /* chat bubbles */
        .stChatMessage[data-testid="stChatMessageUser"]{{ background:var(--chat-user) !important; }}
        .stChatMessage[data-testid="stChatMessageAssistant"]{{ background:var(--chat-assistant) !important; }}

        /* generic primary button minimal look */
        button[kind="primary"]{{ box-shadow:0 0 0 2px var(--accent) inset !important; }}

        /* active chat entry styling */
        .scroll-box button[kind="primary"]{{
            background: var(--active-chat-bg) !important;
            border: 1px solid var(--active-chat-border) !important;
            color: var(--text) !important;
            box-shadow: none !important;
            font-weight:600 !important;
        }}
        .scroll-box button[kind="primary"]:hover{{
            background: var(--active-chat-bg-hover) !important;
            border-color: var(--active-chat-border) !important;
        }}
    ...
    if word_b64_for_sidebar:
        st.sidebar.markdown(
            f"<div class='wbi-wordmark-side'><img src='data:image/png;base64,{word_b64_for_sidebar}'></div>",
            unsafe_allow_html=True
        )
...
# Updated TOOL_DEFINITIONS (around line 1025)

TOOL_DEFINITIONS = f"""
AVAILABLE TOOLS:
- search_knowledge_base(keywords: list[str], semantic_query_text: str, rank_limit: int): Executes a sophisticated **Hybrid Search (RRF)** against the knowledge base. 
    - The LLM MUST provide the atomic keywords (e.g., ["budget", "SFIS"]) and a detailed semantic_query_text (e.g., "Find the project budget for SFIS.").
    - Always set a specific `rank_limit` (e.g., 10).
    - Use this only for data retrieval tasks that require the database.
...
- json_validate(data_json: str, schema_json: str): Performs basic validation of a JSON object against a schema.
...
- schema_diff(a_text: str, b_text: str): Compares two schemas produced by schema_sketch.

OUTPUT FORMAT:
The Tool Agent MUST respond with ONLY a JSON object representing a single tool call:
{{
    "tool_use": {{
        "name": "tool_name", 
        "params": {{"arg1": "value", "arg2": "value"}} 
    }}
}}
"""

AGENT_PERSONAS = {
    "Orchestrator": f"""You are an expert project manager and orchestrator. Your role is to understand the user's goal and guide your team of specialist agents to achieve it.
    ...
    2.  If the scratchpad is empty or only contains the user's goal, your first step is to create a clear, step-by-step plan to achieve the goal.
    ...
    7.  If all steps in the plan are complete and the user's request is fully answered, delegate to the "FINISH" agent with the complete, final answer in the "task" field.
    """,

    "Tool Agent": f"""You are an expert data agent. You can perform all math, data conversion, parsing, validation, and retrieval tasks.

    CRITICAL INSTRUCTIONS:
    1.  Analyze the given task, which will be a question from another agent (e.g., "Calculate 2+2" or "Find the project budget in the DB").
    2.  Determine the single best tool to use to answer the task.
    3.  You MUST respond with ONLY a JSON object representing a single tool call in the specified OUTPUT FORMAT.
    ...
    3.  If you have enough information, perform your analysis and provide the complete technical answer. Respond ONLY with a JSON object containing your answer: {"response": "The analysis shows the memory leak is caused by..."}""",
    ...
    3.  If you have sufficient information, write the full report. Respond ONLY with a JSON object containing your final, well-formatted report: {"response": "# SFIS Project Report..."}""",
    ...
    2.  If you need to cross-reference specific data or perform a validation check, delegate the task to the `Tool Agent` (e.g., {"agent": "Tool Agent", "task": "Use the schema_diff tool on the provided Schema A and Schema B to check for changes"}).
    3.  If you have enough information, provide your validation and recommend the next step. Respond ONLY with a JSON object containing your assessment: {"response": "Validation complete. The data is accurate. The next step should be to have the Writer compile the final report."}"""
}


# --- Replacement for execute_tool_call function (around line 1184) ---

# Global Placeholder Vector (Requires 1536 dimensions for Azure OpenAI)
# In a real app, this would be generated by an Embedding API call on semantic_query_text.
# Using a fixed placeholder vector for demonstration.
...
def execute_tool_call(tool_name: str, params: Dict[str, Any]) -> str:
    ...
    if tool_name == "search_knowledge_base":
        # --- FIX: REVERTING TO SAFE, STANDARD KEYWORD SEARCH (No RRF/Vector) ---
        ...
        # 2. Combine with AND logic for high relevance (must contain ALL keywords)
        where_clause = " AND ".join(contains_clauses)
...
def run_agentic_workflow(user_prompt: str, client: AzureOpenAI, log_placeholder, final_answer_placeholder):
    """
    ...
    for i in range(MAX_LOOPS):
        ...
        log_placeholder.markdown(scratchpad)
        ...
        try:
            with st.spinner(f"Loop {loop_num}: {next_agent} is working on the task..."):
                if next_agent not in AGENT_PERSONAS:
                    ...
                    # --- Execution: Handle Delegation (non-Tool Agent) or Tool Call (Tool Agent) ---
                    if agent_output.get("agent"):
                        # Agent has delegated to another agent (e.g., Engineer -> Tool Agent)
                        ...
                        # Only the Tool Agent can use tools directly. Execute it now.
                        if sub_agent == "Tool Agent":
                            # Call the Tool Agent to get a tool call object
                            ...
                            agent_output = json.loads(response.choices[0].message.content)
                             ...
                             # Re-append the task to the scratchpad so the Orchestrator can process it next.
                             ...
                             continue # Skip to next loop for Orchestrator to pick up
                    ...
                    if "tool_use" in agent_output:
                        ...
                        scratchpad += f"- **Tool Call:** Executing tool `{tool_name}` with parameters: {params}\n"
                        log_placeholder.markdown(scratchpad)
                        
                        # --- 1. Execute Tool Logic (which now returns the SQL string for search_knowledge_base) ---
                        tool_result_observation = execute_tool_call(tool_name, params)
                        
                        # FIX: Check for the new, safe query label
                        if tool_name == "search_knowledge_base" and tool_result_observation.startswith("Tool Observation: Constructed Safe Query:"):
                            
                            # --- 2. Extract Query and Execute RAG Search ---
                            # FIX: Use the new prefix for query extraction
                            ...
                            # Overwrite tool_result_observation with the final search outcome for the scratchpad
                            tool_result_observation = f"Tool Observation (Standard Search): Query executed successfully. {observation}"

                        # --- 3. Handle Other Tool Results and Errors ---
        ...
        if len(observation) > 20000:
            with st.spinner(f"Loop {loop_num}: Summarizing {next_agent}'s findings..."):
                ...
                # Check if the observation is a search result and extract just the content
                if observation.strip().startswith("Found") and "Content:" in observation:
                    try:
                        ...
                        text_to_summarize = observation # Fallback to original text
            ...
            with st.spinner(f"Loop {loop_num}: {next_agent} is working on the task..."):
                if next_agent not in AGENT_PERSONAS:
                    ...
                    if "tool_use" in agent_output:
                        ...
                        if tool_name == "search_knowledge_base" and tool_result_observation.startswith("Tool Observation: Constructed Hybrid Query:"):
                            
                            # --- 2. Extract Query and Execute RAG Search ---
                            sql_query = tool_result_observation.split("Tool Observation: Constructed Hybrid Query:\n", 1)[1]
                            search_results = []
                            selected_kbs = st.session_state.get('selected_containers', [])
                            ...
                            if not selected_kbs:
                                ...
                                with query_expander: st.write("**Hybrid Search Query:**"); st.code(sql_query, language="sql")
                                with st.spinner(f"Loop {loop_num}: Executing Hybrid Search against selected KBs..."):
                                    for kb_path in selected_kbs:
                                        db_name, cont_name = kb_path.split('/')
                                        uploader = get_cosmos_uploader(db_name, cont_name)
                                        if uploader:
                            ...
                            observation = f"Found {len(search_results)} item(s) using Hybrid Search. Content: {json.dumps(search_results, indent=2)}"
                            # Overwrite tool_result_observation with the final search outcome for the scratchpad
                            tool_result_observation = f"Tool Observation (Hybrid Search): Query executed successfully. {observation}"
...
inject_brand_and_theme(circle_b64, word_b64, dark=True)
...
if messages and messages[-1]["role"] == "user":
    ...
    with st.chat_message("assistant"):
        ...
        if persona_type == "agentic":
            try:
                # Initialize the client needed for the agentic workflow
                ...
                final_answer = run_agentic_workflow(user_prompt, agent_client, log_placeholder, final_answer_placeholder)
