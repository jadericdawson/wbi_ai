        ...
        if tool_call_to_execute:
            ...
            try:
                ...
                if tool_name == "search_knowledge_base" and tool_result_observation.startswith("Tool Observation: Constructed Safe Query:"):
                    ...
                    # --- CRITICAL FIX for Repetitive Queries (SMART QUERY) ---
                    if not search_results:
                        current_task_original = task # Capture the original task the Orchestrator sent
                        observation = (
                            f"CRITICAL OBSERVATION: Search for '{current_task_original}' returned **zero results** "
                            f"from the knowledge base. This specific search path has FAILED. "
                            f"The Orchestrator MUST now formulate a new, broader search strategy based on a different SFIS meaning "
                            f"or move to the next planned phase (e.g., Engineer's internal knowledge)."
                        )
                        # Set next agent to Orchestrator with an error message
                        next_agent = "Orchestrator" 
                        task = f"ERROR: The last search (keywords: {params.get('keywords', 'N/A')}) returned zero results. Formulate a new, non-repetitive task to continue Step 2 of the plan."
                        # Skip re-injection of the original tool observation and rely on the new error message
                        scratchpad += f"- **Action Result:** {observation}\n"
                        return # Exit current loop to let the Orchestrator handle the error in the next cycle
                    # --- END CRITICAL FIX ---
