...
AGENT_PERSONAS = {
    ...
    4.  If the scratchpads are empty or only contain the user's goal, your first step is to delegate to Writer to create a plan in the OUTLINE pad.
    ...
    4. **SCRATCHPAD AWARENESS**:
       - When searching/retrieving data: Write results to RESEARCH pad or DATA pad
       - When creating tables: Write to TABLES pad using scratchpad_write()
       - When creating plots: Write to PLOTS pad
       - Use LINE-LEVEL operations for efficiency (scratchpad_insert_lines, etc.)
    ...
    CRITICAL INSTRUCTIONS:
    1.  Use scratchpad tools to read from and write to specialized pads:
        - Read from RESEARCH pad for gathered facts
        - Read from TABLES/PLOTS/DATA pads for visualizations
        - Write to OUTLINE pad for structure/plans
        - Write to OUTPUT pad for the final answer
        - Use LINE-LEVEL editing (scratchpad_insert_lines, scratchpad_replace_lines) to efficiently build content
    2.  When creating outlines/plans: Write to OUTLINE pad, section by section
    3.  When assembling final answers:
        - Read all relevant pads (research, tables, etc.)
        - Build OUTPUT pad incrementally using line-level operations
        - Use scratchpad_get_lines() to review what you've written
    4.  If you need to format data or create tables, delegate to Tool Agent to create them in the TABLES pad
    5.  **CONTENT RULE**: Only use facts from scratchpads. No speculation. Use scratchpad_read() to verify sources.
...
Respond with JSON: {{"agent": "AgentName", "task": "specific task description"}}"""}
        ...
        # Log the orchestrator's decision to scratchpad
        action_log = f"Loop {loop_num}: Orchestrator → {next_agent}: {task[:100]}..."
        scratchpad_mgr.write_section("log", "recent_actions", action_log, mode="append", agent_name="Orchestrator")
